...
tank = MoveTank(left_motor_port=OUTPUT_A, right_motor_port=OUTPUT_D)
while True:

    
    receive = client.pollData().split(",")
    print("recieved", receive)
    try:
        left_DPS, right_DPS, t = map(float, receive)
        print("Moving", SpeedDPS(left_DPS),  SpeedDPS(right_DPS), t)
        
        tank.on_for_seconds(left_speed=SpeedDPS(left_DPS), right_speed=SpeedDPS(right_DPS), seconds=t)
        client.sendDone()
    except:
        print('bad commands recieved')
