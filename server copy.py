...
class Trailer:
    def __init__(self, center, front_hitch, back_hitch, axel_width, orentation):
        self.center = center
    ...
    while len(trailers) > 0:
        ...
        for trailer in trailers: # TODO: the trailer objects need some work to deal with hit1_2 and front back issues
            if trailer[1] == current_hitch or trailer[2] == current_hitch:
                ...
                t_theta = math.atan2(trailer[0].y - front_hitch.y, trailer[0].x - front_hitch.x)

                
                configuration.append(Trailer(trailer[0], front_hitch, back_hitch, trailer[3], t_theta))
...
def vehicle_constructor(tracker):#arguements of some kind):
    ...
    v_theta = math.atan2(V_center_loc.y - back_hitch.y, V_center_loc.x - back_hitch.x)
    #print("v_theta")
    vehicle = Vehicle(V_center_loc, back_hitch, axel_length, v_theta)
    #print("before return vehicle")
    return vehicle
    

def trailer_movement(): # arguements of some kind
    # this function takes in a point and a trailer object. 
    # caluclate the distance from the back hitch to this point.
    # use the kinmenatic model for diferential steering to calculate the omega(t) for the trailer to get the trailers back hitch to that point. 
    # calculate and set the new postion of the center of the trailer
    # calculate and set the new position of the front hitch.
    # set the postion of the back hitch.
    # return the location of the front hitch.
    #TODO
    pass

def vehicle_movement(): #  arguements of some kind
    # this function takes in a point and the vehicle object
    # calculate the disctance from the hitch to the point
    # calculate the kinematic model for differential settering to calculate the v_l and v_r for the vehicle
...
def forward_curved_path():
    #TODO
    # this function will be used to drive the vehicle in a forward curved path with a mode toggle of 0 or 1
    # drives the vehicle in a forward curved path to the right
    # the curve is of radius r
    ...
    # if mode is 1 the vehicle will stop driving after the orientation of the vehicle is pointing away from the goalpoint
    pass

def forward_straight_path():
    #TODO
    # this function will be used to drive the vehicle in a forward straight path with a mode toggle of 0 or 1
    # if mode is 0 the vehicle will stop driving after the vehicle is at least buffer_distance away from the goalpoint
    # if mode is 1 the vehicle will stop driving after driving straight for a distance of Length_of_config
    pass

def forward_readjustment_path():
    #TODO
...
def jackknife_detector(configuration):
    ...
    # the jackknife position is when the units in trailer are at 100 degree angles to the unit behind it
    ...
    for i in range(len(configuration) - 1):
        ...
        # if the angle is greater than 100 degrees return true
        unit_orientation = configuration[i].get_orientation()
        next_unit_orientation = configuration[i + 1].get_orientation()
        angle = abs(unit_orientation - next_unit_orientation)
        if angle > 100:
            return True
    return False
...
def reverse(visual):
    ...
    while goal_detector>goal_threshold:
        temp_configuration = [len(configuration)]
        for unit in range( (len(configuration)-1), 0, -1):
            if unit == len(configuration)-1:
                ...
                last_trailer = reverse_last_trailer_movement(configuration[unit], sub_goal)
        ...
        if jackknife_detector(temp_configuration):
            ...
            v_l = vehicle[1]
            v_r = vehicle[2]
            visual.server.sendStr(f"{v_l}, {v_r}")
...
def calc_sub_goal(back_hitch, goal_point):
    ...
    unit_vector = np.vector([goal_point.x - back_hitch.x, goal_point.y - back_hitch.y])/distance(goal_point, back_hitch)
...
def reverse_last_trailer_movement(unit, subgoal):
    ...
    exit_point = calc_exit_point(unit,subgoal) # gets exit point from curve to align with goal via tangent of curve -> goal
    ...
    unit_orientation = unit.get_orientation()+ theta_si # TODO this may be wrong and we may have to  check that the theta is in the right quadrant
    
    front_hitch = unit.get_front_hitch()
    back_hitch = unit.get_back_hitch()
    trailer_length = distance(front_hitch, back_hitch)
    unit_vector = np.vector([front_hitch.x - back_hitch.x, front_hitch.y - back_hitch.y])/trailer_length
    ...
    new_trailer = Trailer(exit_point, front_hitch, back_hitch, unit.get_axel_width(), unit_orientation)

    return (new_trailer,new_trailer.get_back_hitch())

def reverse_mid_trailer_movement(unit, subgoal):
...
def reverse_vehicle_movement(unit, subgoal):
    ...
    unit_vector = np.vector([center.x - back_hitch.x, center.y - back_hitch.y])/vehicle_length
    back_hitch = Point(exit_point.x - (trailer_length/2)*unit_vector[0], exit_point.y - (trailer_length/2)*unit_vector[1])

    new_vehicle = Vehicle(exit_point, back_hitch, unit.get_axel_width(), unit_orientation)

    arc_len = theta_si*Radius
...
class Tracker:
    ...
    def TrackerThread(self, vehicleColor, trailerColor, hitchColor, goalColor):
        ...
        while rval:
            ...
            self.DrawCircles(frame, trailerPoints, (0, 0, 255))
...
class VisualServoing:
    ...
    def get_path(self, point_pos, goal_pos):
        # TODO: Implement the path planning algorithm
        pass
    def run(self):
        self.capture_positions()
        reverse(self) # or reverse(visual)?
...
#     # No specific objective, so we minimize 0 for a pure constraint satisfaction problem
#     return 0

# def constraints(params, p1, x, p2, r):
#     c_x, c_y, R = params
#     c = np.array([c_x, c_y])
    
#     # Constraint 1: Circle is tangent to the vector x at p1
#     constraint1 = np.linalg.norm(c - p1) - R
    
#     # Constraint 2: Circle passes through p1
#     constraint2 = np.linalg.norm(c - p1) - R  # Redundant but for clarity
    
#     # Constraint 3: Circle perpendicularly intersects the given circle at p2
#     constraint3 = np.linalg.norm(c - p2) - np.sqrt(R**2 + r**2)
    
#     return [constraint1, constraint2, constraint3]

# # Define the inputs
# p1 = np.array([1.0, 0.0])  # Example coordinates of p1
# x = np.array([1.0, 0.0])   # Example unit vector direction at p1
# p2 = np.array([3.0, 3.0])  # Center of the given circle
# r = 2.0                    # Radius of the given circle

# # Initial guess for the optimization: [c_x, c_y, R]
# initial_guess = [2.0, 2.0, 1.0]

# # Define constraints for scipy.optimize
# constraint_funcs = {
#     'type': 'eq',
#     'fun': lambda params: np.array(constraints(params, p1, x, p2, r)),
# }

# # Perform optimization
# result = minimize(objective, initial_guess, constraints=[constraint_funcs], method='SLSQP')

# # Extract results
# if result.success:
#     c_x, c_y, R = result.x
#     print(f"Solution found: Center = ({c_x:.3f}, {c_y:.3f}), Radius = {R:.3f}")
# else:
#     print("Optimization failed. Try different initial guesses or check constraints.")