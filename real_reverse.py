...
class Vehicle:
    def __init__(self, center, back_hitch, axel_width, orientation):
        ...
        self.tan_velocity = 50 # TODO test this value
    ...
    while len(trailers) > 0:
        ...
        for trailer in trailers: # TODO: the trailer objects need some work to deal with hit1_2 and front back issues
            if trailer[1] == current_hitch or trailer[2] == current_hitch:
                ...
                t_vector = unit_vector(trailer[0],back_hitch)
...
def vehicle_constructor(tracker):#arguements of some kind):
    ...
    u_vector = unit_vector(V_center_loc, back_hitch)
...
def jackknife_detector(configuration):
    ...
    tolerance = 5*math.pi/9 # TODO will have to be tested.
    for i in range(len(configuration) - 1):
        ...
        print("jackknife unit vectors: ", unit_vector_1, unit_vector_2)
        ...
        print("jackknife detector angles: ", angle)
        a = math.pi - abs(angle)
...
def reverse(visual):
    ...
    while goal_detector>goal_threshold:
        configuration = visual.capture_positions()
        temp_configuration = [None]*len(configuration)
        for unit in range( (len(configuration)-1), -1, -1):
            if unit == len(configuration)-1:
                sub_goal = calc_sub_goal(configuration[unit], visual.tracker.goalPoint) # point object at x_s, y_s
                ...
                print("REACHED", vehicle)
                temp_configuration[unit] = vehicle[0]
        print("configuration",temp_configuration)
...
def calc_sub_goal(unit, goal_point):
    #TODO
    ...
    current_pos = unit.get_center()
    unit_vector = np.array([goal_point.x - current_pos.x, goal_point.y - current_pos.y])/distance(goal_point, current_pos)
    scalar = distance(unit.get_front_hitch(), unit.get_back_hitch())*0.75
    sub_goal = Point(current_pos.x + scalar*unit_vector[0], current_pos.y + scalar*unit_vector[1])
    return sub_goal
...
def reverse_last_trailer_movement(unit, subgoal, goalPoint):
    ...
    try:
        exit_point,vector_ic = calc_exit_point(unit = unit,subgoal=subgoal, goal=goalPoint, type="last") # gets exit point from curve to align with goal via tangent of curve -> goal
        print("exit point and subgoal: ",exit_point, subgoal)
        print("distance from center to sub goal last", distance(unit.get_center(), exit_point ))
        xi_vec = unit_vector(unit.get_center(),exit_point)

        theta_si = math.acos(xi_vec.x * vector_ic.x + xi_vec.y *vector_ic.y)

        Radius = ((1/2)*distance(exit_point,unit.get_center()))/math.cos(theta_si)

        theta_xi = (math.pi - math.pi/2 - theta_si)*2
        print("thetas", theta_si, theta_xi)
        heading_vector = rotate_vector(unit.get_orientation(), -theta_xi) # TODO this may be wrong and we may have to  check that the theta is in the right quadrant
        print("unit vector", heading_vector)
        front_hitch = unit.get_front_hitch()
        back_hitch = unit.get_back_hitch()
        trailer_length = distance(front_hitch, back_hitch)
        front_hitch = Point(exit_point.x + (trailer_length/2)*heading_vector[0], exit_point.y + (trailer_length/2)*heading_vector[1])
        back_hitch = Point(exit_point.x - (trailer_length/2)*heading_vector[0], exit_point.y - (trailer_length/2)*heading_vector[1])

        new_trailer = Trailer(exit_point, front_hitch, back_hitch, unit.get_axel_width(), heading_vector)

        return (new_trailer,new_trailer.get_back_hitch())
    except:
        d = distance(unit.get_center(), subgoal)
        vec_to_subgoal = unit_vector(unit.get_center(), subgoal)
        new_center = Point(unit.get_center().x + d*vec_to_subgoal.x, unit.get_center().y + d*vec_to_subgoal.y)
        new_front_hitch = Point(unit.get_front_hitch().x + d*vec_to_subgoal.x, unit.get_front_hitch().y + d*vec_to_subgoal.y)
        new_back_hitch = Point(unit.get_back_hitch().x + d*vec_to_subgoal.x, unit.get_back_hitch().y + d*vec_to_subgoal.y)
        new_trailer = Trailer(new_center, new_front_hitch, new_back_hitch, unit.get_axel_width(), unit.get_orientation())
        return (new_trailer,new_trailer.get_back_hitch())
...
def reverse_mid_trailer_movement(unit, subgoal, goalPoint):
    ...
    # this tangent lines up with the goal point
    try:
        exit_point,vector_ic, _ = calc_exit_point(unit,subgoal, goalPoint, "middle" ) # gets exit point from curve to align with goal via tangent of curve -> goal
        xi_vec = unit_vector(unit.get_center(),exit_point)
        print("distance from hitch to sub goal middle", distance(unit.get_center(), exit_point ))
        theta_si = math.acos(xi_vec.x * vector_ic.x + xi_vec.y *vector_ic.y)
        ...
        d = distance(unit.get_back_hitch(), subgoal)
        vec_to_subgoal = unit_vector(unit.get_back_hitch(), subgoal)
        new_center = Point(unit.get_center().x + d*vec_to_subgoal.x, unit.get_center().y + d*vec_to_subgoal.y)
        new_front_hitch = Point(unit.get_front_hitch().x + d*vec_to_subgoal.x, unit.get_front_hitch().y + d*vec_to_subgoal.y)
        new_back_hitch = Point(unit.get_back_hitch().x + d*vec_to_subgoal.x, unit.get_back_hitch().y + d*vec_to_subgoal.y)
        new_trailer = Trailer(new_center, new_front_hitch, new_back_hitch, unit.get_axel_width(), unit.get_orientation())
...
def reverse_vehicle_movement(unit, subgoal, goalPoint):
    ...
    try:
        exit_point,vector_ic, Radius = calc_exit_point(unit,subgoal, goalPoint, "vehicle") # gets exit point from curve to align with goal via tangent of curve -> goal
        Radius = abs(Radius)
        xi_vec = unit_vector(unit.get_center(),exit_point)
        print("distance from hitch to sub goal vehicle", distance(unit.get_center(), exit_point ))
        theta_si = math.acos(xi_vec.x * vector_ic.x + xi_vec.y *vector_ic.y)
        ...
        print("the theta_xi were are going to go,",theta_xi)

        if (unit.get_orientation().x*vector_ic.y - unit.get_orientation().y*vector_ic.x) > 0: 
            side = -1
        ...
        back_hitch = Point(exit_point.x + (vehicle_length/2)*heading_vector[0], exit_point.y + (vehicle_length/2)*heading_vector[1])

        new_vehicle = Vehicle(exit_point, back_hitch, unit.get_axel_width(), heading_vector)

        arc_len = theta_xi*Radius

        print("arc_len the vehicle is moving?", arc_len)

        time = abs(arc_len) / unit.tan_velocity

        omega = theta_xi/time

        
        
        v_l = -omega * (Radius + side*unit.get_axel_width()/2)
        v_r = -omega * (Radius - side*unit.get_axel_width()/2)
        ...
        new_center = Point(unit.get_center().x + d*vec_to_subgoal.x, unit.get_center().y + d*vec_to_subgoal.y)
        new_back_hitch = Point(unit.get_back_hitch().x + d*vec_to_subgoal.x, unit.get_back_hitch().y + d*vec_to_subgoal.y)
        new_vehicle = Vehicle(new_center,  new_back_hitch, unit.get_axel_width(), unit.get_orientation())
        v_l = -unit.tan_velocity
        v_r = -unit.tan_velocity
        time = d/unit.tan_velocity 
